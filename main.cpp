...
bool ArenaGate::checkCollision(int playerX, int playerY, int& newPlayerX, int& newPlayerY) {
    ...
    // Otherwise, the player and gate are overlapping, so we need to correct the player's position
    if (leftA < leftB && rightA > rightB) {
        // Gate is wider than player
        if (abs(playerX - leftA) < abs(rightB - rightA)) {
            // Move the player to the left side of the gate
            newPlayerX = leftA - Player::PLAYER_WIDTH;
        } else {
            // Move the player to the right side of the gate
            newPlayerX = rightA;
        }
    } else if (topA < topB && bottomA > bottomB) {
        // Gate is taller than player
        if (abs(playerY - topA) < abs(bottomB - bottomA)) {
            // Move the player to the top side of the gate
            newPlayerY = topA - Player::PLAYER_HEIGHT;
        } else {
            // Move the player to the bottom side of the gate
            newPlayerY = bottomA;
        }
    }
...
bool Teleporter::checkCollision(int playerX, int playerY, int& newPlayerX, int& newPlayerY) {
    ...
    // Check if the player is outside the teleporter
    if (rightA < leftB || leftA > rightB || bottomA < topB || topA > bottomB) {
        return false;
    }

    // Check if the player is fully inside the teleporter
    if (leftA >= leftB && rightA <= rightB && topA >= topB && bottomA <= bottomB) {
        return true;
    }

    // Otherwise, the player and teleporter are overlapping, so we need to correct the player's position
    if (leftA < leftB && rightA > rightB) {
        // Teleporter is wider than player
        if (abs(playerX - leftA) < abs(rightB - rightA)) {
            // Move the player to the left side of the teleporter
            newPlayerX = leftA - Player::PLAYER_WIDTH;
        } else {
            // Move the player to the right side of the teleporter
            newPlayerX = rightA;
        }
    } else if (topA < topB && bottomA > bottomB) {
        // Teleporter is taller than player
        if (abs(playerY - topA) < abs(bottomB - bottomA)) {
            // Move the player to the top side of the teleporter
            newPlayerY = topA - Player::PLAYER_HEIGHT;
        } else {
            // Move the player to the bottom side of the teleporter
            newPlayerY = bottomA;
        }
    }
