...
const int ARENA_MIN_X = 100;
const int ARENA_MAX_X = 1800;
const int ARENA_MIN_Y = 100;
const int ARENA_MAX_Y = 1800;
...
void spawnEnemy() {
    int x = rand() % (ARENA_MAX_X - ARENA_MIN_X + 1) + ARENA_MIN_X;
    int y = rand() % (ARENA_MAX_Y - ARENA_MIN_Y + 1) + ARENA_MIN_Y;
    Enemy* newEnemy = new Enemy(x, y);
    const char* enemyImagePath = "assets/enemy.png";
    if (!newEnemy->loadTexture(enemyImagePath)) {
...
void resetGame(Player& player, int& score, int& unlockedArena, int& currentArenaEnemyCount, bool& enemyKilled, Uint32& respawnTimer, const std::string& playerName, GameState& gameState) {
    Hub hub;
...
int main(int argc, char* args[]) {
    ...
    if (enemies.empty() && !enemyKilled) {
        for (int i = 0; i < currentArenaEnemyCount; ++i) {
            spawnEnemy();
        }
        enemyKilled = true; 
    }
} else if (gameState == GAME_STATE_NORMAL) {
    deleteAllEnemies(); 
}
        }
            ...
            for (Enemy* enemy : enemies) {
                if (player.checkCollision(*enemy)) {
                    ...
                    if (player.getHealth() <= 0) {
                        resetGame(player, score, unlockedArena, currentArenaEnemyCount, enemyKilled, respawnTimer, playerName, gameState);
                        playerDead = true;
                    }
                }
            }
